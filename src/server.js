...
app.get("/get/:id?", (req, res) => {
    ...
    if (id) {
        ...
        pool.query("SELECT id, companyname, jobtitle, location, TO_CHAR(startdate, 'YYYY-MM-DD') AS startdate, TO_CHAR(enddate, 'YYYY-MM-DD') AS enddate, description FROM workexperience WHERE id = $1", [id], (error, results) => {
            if (error) {
        ...
        pool.query("SELECT id, companyname, jobtitle, location, TO_CHAR(startdate, 'YYYY-MM-DD') AS startdate, TO_CHAR(enddate, 'YYYY-MM-DD') AS enddate, description FROM workexperience ORDER BY enddate DESC, startdate DESC, id DESC", (error, results) => {
            if (error) {
                res.status(500).json({ error: "Database error" });
            }
            else {
...
app.post("/post", (req, res) => {
    ...
    pool.query("INSERT INTO workexperience (companyname, jobtitle, location, startdate, enddate, description) VALUES ($1, $2, $3, $4, $5, $6)", [companyname, jobtitle, location, startdate, enddate, description], (error, results) => {
        if (error) {
...
app.put("/put/:id", (req, res) => {
    ...
    pool.query("UPDATE workexperience SET companyname = $1, jobtitle = $2, location = $3, startdate = $4, enddate = $5, description = $6 WHERE id = $7", [companyname, jobtitle, location, startdate, enddate, description, id], (error, results) => {
        if (error) {
            res
                .status(500)
